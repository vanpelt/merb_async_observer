...
require 'merb_async_observer/util'
  ...
  class AsyncObserver::Worker
    ...
    def startup()
      log_bracketed('worker-startup') do
        ...
        Merb.logger.info "pid is #{$$}"
        Merb.logger.info "app version is #{appver}"
        mark_db_socket_close_on_exec()
        if AsyncObserver::Queue.queue.nil?
          Merb.logger.info 'no queue has been configured'
          exit(1)
        end
      ...
      log_bracketed('worker-get-job') do
        loop do
          begin
            ...
            Merb.logger.info 'Job deadline soon; you should clean up.'
          rescue Exception => ex
            @q_hint = nil # in case there's something wrong with this conn
            Merb.logger.info(
              "#{ex.class}: #{ex}\n" + ex.backtrace.join("\n"))
            Merb.logger.info 'something is wrong. We failed to get a job.'
            Merb.logger.info "sleeping for #{SLEEP_TIME}s..."
            sleep(SLEEP_TIME)
          end
        end
      end
    ...
    def safe_dispatch(job)
      log_bracketed('worker-dispatch') do
        Merb.logger.info "got #{job.inspect}:\n" + job.body
        log_bracketed('job-stats') do
          job.stats.each do |k,v|
            Merb.logger.info "#{k}=#{v}"
          end
        end
        begin
          return dispatch(job)
    ...
    def self.default_handle_error(job, ex)
      Merb.logger.info "Job failed: #{job.server}/#{job.id}"
      Merb.logger.info("#{ex.class}: #{ex}\n" + ex.backtrace.join("\n"))
      job.decay()
    rescue Beanstalk::UnexpectedResponse
    end
  
    def run_ao_job(job)
      Merb.logger.info 'running as async observer job'
    ...
    def run_other(job)
      Merb.logger.info 'trying custom handler'
      self.class.handle.call(job)
    end
  
    def do_all_work()
      Merb.logger.info 'finishing all running jobs. interrupt again to kill them.'
      f = self.class.finish
      f.call() if f
    end
  end
...
class Mysql
  def set_close_on_exec()
    if @net
      ...
      Merb.logger.info "Warning: we are using the C mysql binding, can't set close-on-exec"
    end
  end
end
